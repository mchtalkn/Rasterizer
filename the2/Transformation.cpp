...
Transformation::Transformation(const Rotation& r)
{
    ...
    if(smallest == ABS(r.ux)) { v.ux=0.0; v.y = r.uz; v.z = -1*r.uy; }
    if(smallest == ABS(r.uy)) { v.uy=0.0; v.x = r.uz; v.z = -1*r.ux; }
    if(smallest == ABS(r.uz)) { v.uz=0.0; v.y = r.ux; v.x = -1*r.uy; }
    ...
    double val3[4][4] = {[1,0,0 0],[0, cos(r.angle), -sin(r.angle), 0],[0, sin(r.angle), cos(r.angls
    M =  Matrix4(val2);
    M_rot =  Matrix4(val3);
